--[[
  ██████  AutoFarm Marshmallow – Full Aggressive Version  ██████
  - Bangunan dihapus (housed.0052 & collision parts) untuk performa
  - Pemain dipindahkan ke bawah tanah (di bawah panci)
  - Siklus farming: Water → Sugar Bag → Gelatin → Empty Bag
  - UI Tab: Autofarm, Counters, UIConfig, Credits
  - Keybind untuk Start/Stop
  - Counter otomatis untuk money, materials, general
]]
...
-- ============================================================
-- BUILDING DELETION (AGGRESSIVE OPTIMIZATION)
-- ============================================================
local function deleteBuildingParts()
    local map = workspace:FindFirstChild("Map")
    if not map then return end

    -- 1. Hapus housed.0052 & collision parts di Apartments
    ...
    -- 2. Expand semua ProximityPrompt (jarak tak terbatas)
...
-- Jalankan penghapusan bangunan saat script di-load
task.spawn(function()
    task.wait(2)
...
-- Cek apakah item ada (backpack atau karakter)
...
local function toggleAutofarm()
    if running then
        ...
        goSurface() -- kembali ke permukaan
        if startBtn then startBtn.Text = "Start Autofarm" end
        return
    end

    local qtyBox = qtyInput
    local qty = tonumber(qtyBox and qtyBox.Text or "100")
...
-- ============================================================
-- BUILDING DELETION (DI PANGGIL SAAT START)
-- ============================================================
-- Script asli memanggil penghapusan saat autofarm dimulai
-- Kita panggil juga di sini agar bangunan hilang saat farming
local originalToggle = toggleAutofarm
toggleAutofarm = function()
    if not running then
        -- Hapus bangunan sekali lagi sebelum mulai
        pcall(deleteBuildingParts)
    end
    originalToggle()
end
...
-- Status
statusLabel = makeLabel(afTab, "[autofarm] idle", 0, colors.Text)
...
-- Keybind
makeDivider(afTab, 3)
local keybindLabel = makeLabel(afTab, "Keybind: press a key to bind", 4, colors.SubText)
...
-- Counter state
local counters = { general = nil, money = nil, materials = nil }
...
local function toggleCounter(type)
    ...
    cLayout2.SortOrder = Enum.SortOrder.LayoutOrder
    cLayout2.Padding = UDim.new(0, 3)
    cLayout2.Parent = content
    local cPadding2 = Instance.new("UIPadding")
    cPadding2.PaddingLeft = UDim.new(0, 10)
    cPadding2.PaddingRight = UDim.new(0, 10)
    cPadding2.PaddingTop = UDim.new(0, 8)
    cPadding2.PaddingBottom = UDim.new(0, 10)
    cPadding2.Parent = content

    if type == "general" then
        -- General counter: farm time, bags session, total
        ...
        -- Simpan conn agar bisa di-disconnect
...
local function createToggleButton(text, getter, setter)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- Update theme
        colors = getColors()
        -- Refresh UI colors (sederhana: reload semua frame)
        -- Di sini kita hanya update main frame background dan stroke
        mainFrame.BackgroundColor3 = colors.MainBG
        border.Color = colors.Border
        titleBar.BackgroundColor3 = colors.BarBG
        titleLabel.TextColor3 = colors.Text
        ...
        tabs[firstTab].img.ImageColor3 = colors.Text
        tabs[firstTab].lbl.TextColor3 = colors.Text
        -- Update background transparansi main frame
...
makeLabel(credTab, "Quickz Autofarm v2.0", 0, colors.Text)
makeLabel(credTab, "Developed by: Quickz", 1, colors.SubText)
makeLabel(credTab, "Discord: discord.gg/WdTbHzcqpU", 2, colors.SubText)

-- ============================================================
-- UPDATE CANVAS SIZE SAAT TAB DIKLIK
-- ============================================================
...
-- SELESAI
-- ============================================================
print("[AutoFarm] Script loaded successfully. UI is ready.")