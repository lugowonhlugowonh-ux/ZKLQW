...
local function StartKillAura()
    ...
    KillAuraConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Simulate attack (tool activation / clickdetector)
...
local DefaultLocations = {
    ["Dealer Ship"] = Vector3.new(0, 10, 0), -- Ganti dengan koordinat asli
    ["Bypass Teleport"] = Vector3.new(50, 5, 50) -- Contoh lokasi
}

-- Load default locations
for name, pos in pairs(DefaultLocations) do
...
local function OpenPlayerTeleportMenu()
...
local CombatSection = CombatPage:CreateSection("Combat Settings")
CombatSection:AddToggleWithBind("Enable Kill Aura", false, Enum.KeyCode.Z, function(state)
...
CombatSection:AddSlider("Aura Radius", 50, 1000, KillAuraCFG.RADIUS, function(val)
    KillAuraCFG.RADIUS = val
end)
CombatSection:AddToggle("Team Check", false, function(state)
    KillAuraCFG.TEAM_CHECK = state
end)
CombatSection:AddToggle("Wall Check", false, function(state)
...
local hitboxIndex = 1
local hitboxFrame = Create("Frame", {Parent = CombatSection.Container, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 24)})
...
-- // KILL AURA TAB (opsional, bisa juga di combat)
local KillAuraTab = Window:CreateTab("Kill Aura")
local KillAuraPage = KillAuraTab:CreatePage("Kill Aura")
local KASection = KillAuraPage:CreateSection("Kill Aura")
-- Bisa duplikat atau tambahan, tapi kita sudah punya di Combat, jadi biarkan kosong atau isi ulang

-- // TELEPORT TAB
local TeleportTab = Window:CreateTab("Teleport")
local TeleportPage = TeleportTab:CreatePage("Teleport")

local TeleportSection = TeleportPage:CreateSection("Teleport Locations")

-- Daftar lokasi default
local function RefreshLocationList()
    -- Hapus item yang sudah ada (kecuali section title dan tombol)
    for _, child in ipairs(TeleportSection.Container:GetChildren()) do
        if child:IsA("Frame") and child.Name == "LocItem" then
            child:Destroy()
        end
    end
    ...
    for name, pos in pairs(SavedLocations) do
        local LocFrame = Create("Frame", {Parent = TeleportSection.Container, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 30), Name = "LocItem"})
        ...
        DelBtn.MouseButton1Click:Connect(function()
            -- Jangan hapus default locations
            if DefaultLocations[name] then
                Library:Notify({Title = "Warning", Description = "Cannot delete default location."})
                return
            end
...
-- Tombol "Select Location" untuk teleport ke player
TeleportSection:AddButton("Select Location", function()
    OpenPlayerTeleportMenu()
end)

-- Tombol "Save Current Location"
...
-- Tombol Dealer Ship (sebagai shortcut)
TeleportSection:AddButton("Dealer Ship", function()
    TeleportToLocation("Dealer Ship")
end)
...
ThemeSection:AddColorPicker("Accent Color", AccentColor, function(color)
    ...
    -- Update stroke lainnya jika perlu
end)
