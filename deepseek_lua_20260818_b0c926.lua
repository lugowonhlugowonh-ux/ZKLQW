...
function Library:CreateWindow(options)
    ...
    local PopupTitle = Create("TextLabel", {Parent = PopupCard, Text = "exit DARK HUB", Font = Enum.Font.GothamBold, TextSize = 18, TextColor3 = TextColor, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 30), Position = UDim2.new(0, 0, 0, 25), ZIndex = 102, TextTransparency = 1, TextXAlignment = Enum.TextXAlignment.Center})
    ...
    function Window:CreateTab(tabName, isDefault)
        ...
        function TabConfig:CreatePage(pageName)
            ...
            function PageObj:CreateSection(sectionName)
                ...
                function Elements:AddToggleWithColor(name, default, colorDefault, callback, colorCallback)
                    local state = default
                    local color = colorDefault
                    local h, s, v_hsv = color:ToHSV()
                    local dropped = false

                    local CFrame = Create("Frame", {Parent = ItemContainer, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 24), ClipsDescendants = true})
                    Create("TextLabel", {Parent = CFrame, Text = name, Font = Enum.Font.Gotham, TextSize = 12, TextColor3 = SubTextColor, BackgroundTransparency = 1, Size = UDim2.new(1, -130, 0, 24), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = Enum.TextXAlignment.Left})
                    
                    local DisplayBtn = Create("TextButton", {Parent = CFrame, Text = "", BackgroundColor3 = color, Size = UDim2.new(0, 30, 0, 16), Position = UDim2.new(1, -86, 0.5, -8), AutoButtonColor = false})
                    Create("UICorner", {Parent = DisplayBtn, CornerRadius = UDim.new(0, 4)})
                    Create("UIStroke", {Parent = DisplayBtn, Color = Color3.fromRGB(255,255,255), Transparency = 0.8, Thickness = 1})
                    AddBounce(DisplayBtn)

                    local Lever = Create("TextButton", {Parent = CFrame, Text = "", BackgroundColor3 = state and AccentColor or Color3.fromRGB(45, 45, 50), Size = UDim2.new(0, 36, 0, 18), Position = UDim2.new(1, -46, 0.5, -9), AutoButtonColor = false})
                    Create("UICorner", {Parent = Lever, CornerRadius = UDim.new(1, 0)})
                    AddBounce(Lever)
                    local Knob = Create("Frame", {Parent = Lever, BackgroundColor3 = Color3.fromRGB(255, 255, 255), Size = UDim2.new(0, 14, 0, 14), Position = state and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)})
                    Create("UICorner", {Parent = Knob, CornerRadius = UDim.new(1, 0)})

                    local PickerArea = Create("Frame", {Parent = CFrame, BackgroundColor3 = BackgroundColor, Size = UDim2.new(1, -20, 0, 110), Position = UDim2.new(0, 10, 0, 30)})
                    Create("UICorner", {Parent = PickerArea, CornerRadius = UDim.new(0, 4)})
                    ...
                    PickerCloseBtn.MouseButton1Click:Connect(function()
                        dropped = false
                        Tween(CFrame, {Size = UDim2.new(1, 0, 0, 24)}, 0.3)
                    end)
                    ...
                    local function internalSetColor(hexString)
                        if not string.find(hexString, "^#") then hexString = "#" .. hexString end
                        local s_check, c = pcall(function() return Color3.fromHex(hexString) end)
                        if s_check and c then
                            color = c
                    ...
                    HexBox.FocusLost:Connect(function() internalSetColor(HexBox.Text) end)
                    
                    DisplayBtn.MouseButton1Click:Connect(function()
                        dropped = not dropped
                        Tween(CFrame, {Size = UDim2.new(1, 0, 0, dropped and 150 or 24)}, 0.3)
                    end)
...
if not CastBlacklist or not CastWhitelist then
    warn("[WARNING] Fungsi CastBlacklist/CastWhitelist tidak ditemukan! Wallbang mungkin tidak work di game ini.")
else
...
-- // ATHER-STYLE UI
-- The existing gameplay/feature logic above is kept intact.
-- Only the UI layer below is replaced.

local AtherLibrary = loadstring(game:HttpGet(
    "https://raw.githubusercontent.com/NIcoGabrielRealYtr/Ather-Hub-Library/refs/heads/main/Source"
))()

local Window = AtherLibrary:Window({
    Name = '<font color="rgb(175, 102, 126)">DARK</font> HUB',
    Logo = "rbxassetid://133425623304338"
})

-- Separate GUI used by the existing whitelist feature.
local CompatScreenGui = Instance.new("ScreenGui")
CompatScreenGui.Name = "DARKHUB_whitelist_ui"
CompatScreenGui.ResetOnSpawn = false
CompatScreenGui.IgnoreGuiInset = true
CompatScreenGui.Parent = CoreGui

-- Keep these references available for the existing whitelist/settings logic.
Window.ScreenGui = CompatScreenGui
Window.ConfigElements = {}

-- // WHITELIST WINDOW
local WhitelistCard = Create("Frame", {
    Parent = CompatScreenGui,
...
Create("UICorner", {Parent = WhitelistCard, CornerRadius = UDim.new(0, 10)})
local WLStroke = Create("UIStroke", {
    Parent = WhitelistCard,
    Color = AccentColor,
    Thickness = 1
})
local WLScale = Create("UIScale", {Parent = WhitelistCard, Scale = 0.8})

local WLHeader = Create("Frame", {
    Parent = WhitelistCard,
    BackgroundColor3 = CardColor,
    Size = UDim2.new(1, 0, 0, 45),
    ZIndex = 201
})
Create("UICorner", {Parent = WLHeader, CornerRadius = UDim.new(0, 10)})
...
local WLCloseBtn = Create("TextButton", {
    Parent = WLHeader,
    Text = "×",
    Font = Enum.Font.GothamBold,
    TextSize = 18,
...
local WLSearchBox = Create("TextBox", {
    Parent = WhitelistCard,
    BackgroundColor3 = CardColor,
...
Create("UIPadding", {Parent = WLSearchBox, PaddingLeft = UDim.new(0, 10)})
Create("UICorner", {Parent = WLSearchBox, CornerRadius = UDim.new(0, 6)})
Create("UIStroke", {
    Parent = WLSearchBox,
...
local WLContainer = Create("ScrollingFrame", {
    Parent = WhitelistCard,
    BackgroundTransparency = 1,
...
local function PopulateWhitelist()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if filterText ~= ""
            and not string.find(plr.Name:lower(), filterText)
            and not string.find(plr.DisplayName:lower(), filterText) then
            continue
        end
        ...
        Create("UICorner", {Parent = ItemFrame, CornerRadius = UDim.new(0, 6)})
        Create("UIStroke", {
            Parent = ItemFrame,
        ...
        local ToggleBtn = Create("TextButton", {
            ...
            Font = Enum.Font.GothamBold,
            TextSize = 14,
            TextColor3 = isWL and Color3.fromRGB(255, 255, 255) or SubTextColor,
            BackgroundColor3 = isWL and AccentColor or Color3.fromRGB(40, 40, 45),
        ...
        ToggleBtn.MouseButton1Click:Connect(function()
            ...
            Tween(ToggleBtn, {
                BackgroundColor3 = state and AccentColor or Color3.fromRGB(40, 40, 45),
                TextColor3 = state and Color3.fromRGB(255, 255, 255) or SubTextColor
            }, 0.2)
        end)
    end
...
WLSearchBox:GetPropertyChangedSignal("Text"):Connect(PopulateWhitelist)

WLCloseBtn.MouseButton1Click:Connect(function()
    Tween(WLScale, {Scale = 0}, 0.25)
    task.wait(0.25)
...
local function OpenWhitelistWindow()
    ...
    Tween(WLScale, {Scale = 0.8}, 0.3)
end

Players.PlayerAdded:Connect(function()
    if WhitelistCard.Visible then
        PopulateWhitelist()
    end
end)
...
local CombatSub = CombatPage:SubPage({
    Name = "Aimbot",
    Icon = "lucide:target",
    DisplayName = true
})

local VisualSub = VisualsPage:SubPage({
    Name = "ESP",
    Icon = "lucide:eye",
    DisplayName = true
})

local MainSection = CombatSub:Section({
    Name = "Main Settings",
    Side = 1
})

local UtilitySection = CombatSub:Section({
    Name = "Utilities",
    Side = 2
})

local VisualSection = VisualSub:Section({
    Name = "Visual Options",
    Side = 1
})

-- Silent Aim
local silentAimToggle = MainSection:Toggle({
...
-- Noclip
local noclipToggle = UtilitySection:Toggle({
    Name = "Noclip",
    Flag = "NoclipEnabled",
    Default = false,
    Callback = function(Value)
...
ThemeSection:Label({
    ...
    Callback = function(Color)
        AccentColor = Color
        WLStroke.Color = Color
    end
})

Window:Notify({
    Title = "Welcome",
    Description = "DARK HUB UI loaded.",
    Duration = 3
})

getgenv().DarkHubWindow = Window
getgenv().LuzorLibrary = AtherLibrary
